...
from src.models import (
    ...
    BatchManager, CostManager, RecipeTableModel
)
...
from PyQt6.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QLabel, QTabWidget, QHeaderView
class MainWindow(QMainWindow):
    ...
    def connect_signals(self):
            ...
            self.recipe_tab.new_btn.clicked.connect(self.controller.on_new_clicked)
    ...
    def on_oil_added(self):
        """
        If you are adding an oil manually, the model needs to know the internal
        dictionary 'self.calculator.oils' has changed size.
        """
        if hasattr(self.view, 'recipe_model'):
            self.view.recipe_model.beginResetModel()
            # No need to change calculator here, just notify of the data change
            self.view.recipe_model.endResetModel()
            print("[DEBUG] Manual oil add: Model reset triggered")
